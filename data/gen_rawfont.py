...
def main() -> int:
    ...
    parser.add_argument("-t", "--table", type=str, help="output table filename (.rftable)")
    parser.add_argument("-y", action='store_true', help="overwrite existing output file")
    parser.add_argument("--preview", action='store_true', help="print output to stdout then exit")

    g = parser.add_mutually_exclusive_group()
    g.add_argument("--complex", action='store_true', help="output complex (.rawfontc) format")
    g.add_argument("--reduced", action='store_true', help="output reduced (.rawfont) format")
    ...
    print("Generating {}rawfont for '{}' @ {} pt...".format(
    ...
    if args.reduced:
        ...
        if args.table:
            print("Warning: unused option '--table' present when generating complex rawfont")
    ...
    if write_rawfont(out_file, raw_font, table_filename=table_file):
        print("Complete: {}rawfont written to output file '{}'.".format(
            f"{rf_text_type.title()} " if rf_text_type != '' else '',
            out_file
        ))
        if uses_table:
            print("Complete: {}rftable written to output file '{}'.".format(
                f"{rf_text_type.title()} " if rf_text_type != '' else '',
                table_file
            ))
        return 0
...
class Writer:
    ...
    def write_char(self, v):
        return self.file_handle.write(
            struct.pack('c' * len(v), *[c for c in v])
        )
    ...
    def write_u16(self, v):
        return self.file_handle.write(
            struct.pack(self.endian_marker + 'H', v)
        )

    def write_u32(self, v):
        return self.file_handle.write(
            struct.pack(self.endian_marker + 'I', v)
        )

    def write_u64(self, v):
        return self.file_handle.write(
            struct.pack(self.endian_marker + 'Q', v)
        )
...
def write_complex_rawfont(filename:str, rf: RawFont, table_filename: Optional[str]=None):
    def end_section():
